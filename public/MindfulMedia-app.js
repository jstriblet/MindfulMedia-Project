...
function renderVideos (videos, writediv) {
  ...
  function onPlayerStateChange(event) {
    ...
    // Fade when video has played completely
    if(event.data === 0) {            
...
async function createUser(user) {
  let userId = user.Ca
  let userObject = await firebase.database().ref('users/' + userId).once('value')
  let userDetails;

console.log(userObject)

  if (val in userObject) {
    (async function(){
      userDetails = await firebase.database().ref('users/' + userId).once('value');
    })()
  }
  else {
